...
function VentasManager() {
  ...
  const handleVerDetalles = (id) => {
    fetch(`http://localhost:3001/ventas/${id}/detalles`)
  ...
  const columns = [
    ...
    { field: 'estado_pago', headerName: 'Estado Pago', width: 120 },
    {
      field: 'actions',
      headerName: 'Detalles',
      sortable: false,
      width: 100,
      renderCell: (params) => (
        <IconButton onClick={() => handleVerDetalles(params.id)} color="primary">
          <VisibilityIcon />
        </IconButton>
      ),
    },
        ...
        <DialogContent>
          <List>
            {detallesVenta.map((detalle, index) => (
              <ListItem key={index}>
                <ListItemText primary={`${detalle.cantidad}x ${detalle.producto_nombre}`} secondary={`Precio unitario: $${Number(detalle.precio_unitario).toLocaleString('es-CO')}`} />
              </ListItem>
            ))}
          </List>
        </DialogContent>
